...
var commands = {
	...
	machineId: "/usr/bin/hostnamectl | grep 'Machine ID:' | cut -d ':' -f 2 | tr -d ' \n'",
	hostname: "/bin/hostname | tr -d '\n'",
	clockSpeed: '/opt/vc/bin/vcgencmd measure_clock ',
	displayState: 'export DISPLAY=:0;/usr/bin/xset dpms force ',
}
...
var system = {
     ...
     */	
    machineId: function () {
        return execSync(commands.machineId).toString();
    },
     ...
     */	
    hostname: function () {
        return execSync(commands.hostname).toString();
    },
	 ...
	 *
	 * @param {boolean} enable - turn on (true), or off (false)
	 */
	displayState: function(enable) {
		var state = (enable) ? 'on' : 'off';
		execSync(commands.displayState + state).toString();
	}
